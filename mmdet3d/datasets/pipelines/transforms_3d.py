...
class ResizePose(Resize):
    """
        scale_depth (int): Whether to scale human depth according to image size.
        abs_dz (int) Whether to keep relative depth of each body joint unchanged when scaling human depth.
    """
    def __init__(self, scale_depth=False, abs_dz=False, **kwargs):
        super(ResizePose, self).__init__(**kwargs)
        self.scale_depth = scale_depth
        self.abs_dz = abs_dz
        if self.abs_dz:
            assert scale_depth

    def _resize_pose(self, results):
        """Resize bounding boxes with ``results['scale_factor']``."""
        scale_factor = results['scale_factor'][:2]     # w, h
        num_joints = (results['gt_poses_3d'].shape[-1] - 3) // 4
        centers2d = results['centers2d']
        depths = results['depths']
        assert (centers2d == results['gt_poses_3d'][:, :2]).all()
        assert (depths == results['gt_poses_3d'][:, 2]).all()
        joints = results['gt_poses_3d'][:, 3:3 + num_joints * 3].reshape(-1, num_joints, 3)
        joints_vis = results['gt_poses_3d'][:, 3 + num_joints * 3:].reshape(-1, num_joints)
        # rescale
        centers2d = centers2d * scale_factor
        joints[..., :2] = joints[..., :2] * scale_factor
        if self.scale_depth:
            depth_scale = np.sqrt(scale_factor.prod())
            depths = depths / depth_scale
            if not self.abs_dz:
                joints[..., 2] = joints[..., 2] / depth_scale
        # update
        gt_poses_3d = np.concatenate([
            centers2d, depths.reshape(-1, 1), joints.reshape(-1, num_joints * 3), joints_vis
        ], axis=-1)
        results['centers2d'] = centers2d
        results['depths'] = depths
        results['gt_poses_3d'] = gt_poses_3d

    def __call__(self, results):
        super(ResizePose, self).__call__(results)
...
@PIPELINES.register_module()
class RandomFlipPose3D(RandomFlip):
    """Flip the human pose.
    ...
    Args:
        ...
        flip_ratio_bev_vertical (float, optional): The flipping probability
            in vertical direction. Defaults to 0.0.
        num_joints (int): The number of body joints.
        flip_pairs (List[List]): The left-right pairs of body joints.
    """
    def __init__(self,
                 sync_2d=True,
                 flip_ratio_bev_horizontal=0.0,
                 ...
                 flip_pairs=None,
                 **kwargs):
        super(RandomFlipPose3D, self).__init__(
            flip_ratio=flip_ratio_bev_horizontal, **kwargs)
        self.sync_2d = sync_2d
        self.flip_ratio_bev_vertical = flip_ratio_bev_vertical
        if flip_ratio_bev_horizontal is not None:
            assert isinstance(
                flip_ratio_bev_horizontal,
                (int, float)) and 0 <= flip_ratio_bev_horizontal <= 1
        assert flip_ratio_bev_vertical == 0
        if flip_ratio_bev_vertical is not None:
            assert isinstance(
                flip_ratio_bev_vertical,
                (int, float)) and 0 <= flip_ratio_bev_vertical <= 1
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        Returns:
            dict: Flipped results, 'points', 'bbox3d_fields' keys are \
                updated in the result dict.
        """
        assert direction in ['horizontal'] and self.sync_2d

        if 'gt_poses_3d' in input_dict:
            w = input_dict['img_shape'][1]

            centers2d = input_dict['centers2d']
            depths = input_dict['depths']
            joints = input_dict['gt_poses_3d'][:, 3:3+self.num_joints*3].reshape(-1, self.num_joints, 3)
            joints_vis = input_dict['gt_poses_3d'][:, 3+self.num_joints*3:].reshape(-1, self.num_joints)
            # Flip horizontal
            centers2d[..., 0] = w - centers2d[..., 0] - 1
            joints[:, :, 0] = w - joints[:, :, 0] - 1
            ...
            # print(centers2d.shape, depths.shape, joints.shape, joints_vis.shape)
            gt_poses_3d = np.concatenate([
                centers2d, depths.reshape(-1, 1), joints.reshape(-1, self.num_joints*3), joints_vis
            ], axis=-1)

            input_dict['centers2d'] = centers2d
            input_dict['gt_poses_3d'] = gt_poses_3d
            return
    ...
    def __call__(self, input_dict):
        # filp 2D image and its annotations
        super(RandomFlipPose3D, self).__call__(input_dict)
...
@PIPELINES.register_module()
class GlobalRotScaleTransPose(object):
    """Apply global rotation, scaling and translation to a 3D scene.
    ...
    Args:
        ...
        scale_depth (int): Whether to scale human depth according to image size.
        abs_dz (int) Whether to keep relative depth of each body joint unchanged when scaling human depth.
        img_norm_cfg: Image normalize config.
        use_bbox_center: Whether to use box center as human center.
    """
    ...
    def __init__(self,
        ...
        if img_norm_cfg is not None:
            self.img_mean = img_norm_cfg['mean']
            if img_norm_cfg['to_rgb']:
                self.img_mean = self.img_mean[::-1]
        else:
            self.img_mean = [127.5, 127.5, 127.5]

        self.use_bbox_center = use_bbox_center

    def _transform(self, input_dict):
        h, w, _ = input_dict['img_shape']

        scale = input_dict['pcd_scale_factor']

        rot = input_dict['pcd_rot']
        trans_factor = input_dict['pcd_trans']
        center = np.array([w/2, h/2], dtype=np.float)
        center = center * (1 + trans_factor)
        new_scale = np.array([w, h], dtype=np.float) * scale
        trans = get_affine_transform(center, new_scale, rot, [w, h])

        for key in input_dict.get('img_fields', ['img']):
            img = input_dict[key]
            inp_h, inp_w = img.shape[:2]
            assert inp_h == h and inp_w == w
            img_new = cv2.warpAffine(img, trans, (int(inp_w), int(inp_h)), flags=cv2.INTER_LINEAR, borderValue=self.img_mean)
            input_dict[key] = img_new

        # including center and joints
        joints = input_dict['gt_poses_3d'][:, :3+self.num_joints*3]
        batch = len(joints)
        joints_vis = input_dict['gt_poses_3d'][:, 3+self.num_joints*3:].reshape(batch, self.num_joints)
        joints = joints.reshape(batch, self.num_joints+1, 3)
        joints_depth = joints[..., [2]]
        if self.scale_depth:
            if not self.abs_dz:
                joints_depth = joints_depth * scale
            else:
                joints_depth[0] = joints_depth[0] * scale
        # deal with joints visibility

        new_joints = joints.copy()
        new_joints[..., 2] = 1
        new_joints = new_joints.reshape(-1, 3)
        new_joints = np.dot(new_joints, trans.T)
        new_joints = new_joints.reshape(batch, -1, 2)
        new_joints = np.concatenate([new_joints, joints_depth], axis=-1)

        gt_poses_3d = np.concatenate([
            new_joints.reshape(batch, -1), joints_vis
        ], axis=-1).astype(np.float32)
        assert ((0 <= joints_vis) & (joints_vis <= 1)).all()

        # warp 2d bboxes
        gt_bboxes = input_dict['gt_bboxes']
        new_gt_bboxes = gt_bboxes.reshape(batch, 2, 2)
        new_gt_bboxes_rev = new_gt_bboxes.copy()
        new_gt_bboxes_rev[..., 1] = new_gt_bboxes[..., 1][:, ::-1]
        new_gt_bboxes = np.concatenate([new_gt_bboxes, new_gt_bboxes_rev], axis=1)
        new_gt_bboxes = np.concatenate([new_gt_bboxes, np.ones([batch, 4, 1])], axis=-1).reshape(-1, 3)
        new_gt_bboxes = np.dot(new_gt_bboxes, trans.T)[:, :2].reshape(batch, 4, 2)
        xmin_ymin = new_gt_bboxes.min(axis=1)
        xmax_ymax = new_gt_bboxes.max(axis=1)
        new_gt_bboxes = np.concatenate([xmin_ymin, xmax_ymax], axis=-1) # [n, (x1,y1,x2,y2)]
        new_gt_bboxes[:, 0::2] = new_gt_bboxes[:, 0::2].clip(0, w-1)
        new_gt_bboxes[:, 1::2] = new_gt_bboxes[:, 1::2].clip(0, h-1)
        input_dict['gt_bboxes'] = new_gt_bboxes

        input_dict['centers2d'] = gt_poses_3d[:, :2]
        input_dict['depths'] = gt_poses_3d[:, 3]
        input_dict['gt_poses_3d'] = gt_poses_3d

        # deal with invisible joints
        out_of_bound = (new_joints[..., 0] < 0) | (new_joints[..., 0] > w - 1) | \
                       (new_joints[..., 1] < 0) | (new_joints[..., 1] > h - 1) # [batch, num_joints+1]
        joints_vis[out_of_bound[:, 1:]] = 0
        if not self.use_bbox_center:
            # if root is out of image, take this person as invalid one
            valid = ~out_of_bound[:, 0]
            gt_poses_3d = np.concatenate([
                new_joints.reshape(batch, -1), joints_vis
            ], axis=-1).astype(np.float32)
            if valid.sum() < 2:
            ...
            # adjust bbox center according to warpped bbox
            new_centers = np.stack([new_gt_bboxes[:, 0::2].mean(-1), new_gt_bboxes[:, 1::2].mean(-1),
                                    new_joints[:, 0, -1]], axis=-1)
            # new_centers = new_joints[:, 0].copy()
            # new_centers[:, 0] = new_centers[:, 0].clip(0, w-1)
            # new_centers[:, 1] = new_centers[:, 1].clip(0, h-1)
            gt_poses_3d = np.concatenate([
        ...
        input_dict['depths'] = gt_poses_3d[:, 2][valid].copy()
        input_dict['gt_labels'] = input_dict['gt_labels'][valid]
        input_dict['gt_labels_3d'] = input_dict['gt_labels_3d'][valid]

        input_dict['transform_mat'] = trans

        return input_dict

    def _trans_points(self, input_dict):
        translation_std = np.array(self.translation_std, dtype=np.float32)
        trans_factor = np.random.normal(scale=translation_std, size=2).T
        input_dict['pcd_trans'] = trans_factor
    ...
    def _rot_points(self, input_dict):
        ...
        rot_mat_T = np.array([[rot_cos, -rot_sin, 0],
                              ...
                              [0, 0, 1]]).T
        input_dict['pcd_rotation'] = rot_mat_T
        input_dict['pcd_rot'] = noise_rotation / math.pi * 180
    ...
    def __call__(self, input_dict):
        ...
        assert 'pcd_scale_factor' not in input_dict
        self._random_scale(input_dict)
